...
export default function NavBar({ navlinks }) {
    const navheight = getComputedStyle(document.documentElement).getPropertyValue('--my-variable-name');
            ...
            <div className="collapse navbar-collapse">
                <div className="container-fluid">
                    ...
                    <div className="navbar-nav mr-auto">
                        {navlinks.map((link, i) => {
                            return (
                                <Link key={"link-" + i} className={"nav-link " + ((link.id === "header") ? "d-none" : "")} to={link.id} offset={(link.id==='viaggio') ? -35 : -135} spy={true} smooth={true} duration={200} onSetActive={handleSetActive}>
