...
export default function NavBar({ navlinks }) {
    const navheight = getComputedStyle(document.documentElement).getPropertyValue('--myheight-navbar');
    console.log("navheight: ",parseInt(navheight));
            ...
            if (to === 'viaggio') { setCurrentLink('viaggio'); window.history.replaceState(null, null, "/#viaggio"); }
                ...
                if (to === 'referente') { setCurrentLink('referente'); window.history.replaceState(null, null, "/#referente"); }
                    ...
                    if (to === 'mappa') { setCurrentLink('mappa'); window.history.replaceState(null, null, "/#mappa"); }
                        ...
                        if (to === 'header') {setCurrentLink(''); window.history.replaceState(null, null, "/");}
                    }
                }
            }
        }
            ...
            <div className="collapse navbar-collapse">
                <div className="container-fluid">
                    ...
                    <div className="navbar-nav mr-auto">
                        {navlinks.map((link, i) => {
                            return (
                                <Link key={"link-" + i} className={"nav-link " + ((link.id === "header") ? "d-none" : "")} to={link.id} offset={(link.id==='viaggio') ? -35 : -35} spy={true} smooth={true} duration={200} onSetActive={handleSetActive}>
