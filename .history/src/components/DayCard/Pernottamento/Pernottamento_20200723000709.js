                ...
                return (
                    <div className='row' key={"accomodation" - i}>
                        ...
                        <div className='col-12 col-lg-5'>
                            <p className="title">{acc.name} {(acc.stars.includes('Lux') ? acc.stars.split('_')[0]+"*L" : acc.stars + '*')}</p>
                            <button className='btn btn-outline-primary m-0' disabled>{acc.places[0].name}</button>
                            <p className="text-small"><span className='taglia-testo-pernottamento' >{acc.description}</span><Link to="" onClick={() => utils.funzioneApriModale(setModPernottamento, modPernottamento)}>Scopri di più</Link></p>
