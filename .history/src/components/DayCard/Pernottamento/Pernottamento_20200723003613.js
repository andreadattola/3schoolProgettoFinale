...
export default function Pernottamento({ numeroGiorni, booleano }) {
    ...
    return (
        <>
            ...
            {!booleano && <div><p className='title nomobile'>Ricordati che pernotti al 
            <span className='text-pernottamento'>{acc.name} {(acc.stars.includes('Lux') ? acc.stars.split('_')[0]+"*L" : acc.stars + '*')}</span></p></div>}
        </>
    )
}
