                ...
                <div className="col-12 text-center">
                    <div className="row flex-column align-items-center text-center">
                        ...
                        {Array(numeroAttivita).fill(null).map((att,i) => {
                            return <div key={"icon-act-"+i} className='col-12 text-center'>
                                <div className="activity-circle"><FontAwesomeIcon icon={faMapMarker} /></div></div>
                        })}
                        {transports.map((tr,i) => {
                            return <div key={"icon-tr-"+i} className='col-12 text-center'>
                                <div className="div-transport">
                                    <FontAwesomeIcon className={"myicon-padding "+arrayAssociativo[tr.typology].classe + " transport-circle"} icon={arrayAssociativo[tr.typology].icon} />
                                </div>
                            </div>
                        })}
                    </div>
