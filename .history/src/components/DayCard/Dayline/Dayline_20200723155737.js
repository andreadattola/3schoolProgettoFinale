                    ...
                    <div className="row flex-column align-items-center">
                        ...
                        {Array(numeroAttivita).fill(null).map((att,i) => {
                            return <div key={"icon-act-"+i} className='col-12 col-md-8 offset-md-2 mobile-padding-div-icon'>
                                <div className="activity-circle"><FontAwesomeIcon className="dayline-icon-padding" icon={faMapMarker} /></div></div>
                        })}
                        {transports.map((tr,i) => {
                            return <div key={"icon-tr-"+i} className='col-12 col-md-8 offset-md-2 mobile-padding-div-icon'>
                                <div className="div-transport">
                                    <FontAwesomeIcon className={"dayline-icon-padding "+arrayAssociativo[tr.typology].classe + " transport-circle"} icon={arrayAssociativo[tr.typology].icon} />
                                </div>
                            </div>
                        })}
                    </div>
